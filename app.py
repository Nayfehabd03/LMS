...
# Classes with database integration
class User:
...
class Book:
    ...
    def return_book(book_id, user_id):
        ...
        if borrowed:
            due_date = datetime.strptime(borrowed[4], '%Y-%m-%d')  # Due date of the book
            now = datetime.now()
            overdue_days = (now - due_date).days  # Calculate overdue days

            # If the book is overdue, calculate the fine
            if overdue_days > 0:
                fine = overdue_days * 2  # $2 fine per day
            ...
            # Update book status and remove it from BorrowedBook table
...
def reserve_room(user_name):
    ...
    try:
        ...
        # Check if the room is already reserved for the given time slot
        cursor.execute(
...
def cancel_room_reservation(user_name):
    ...
    """
    try:
        # Fetch all reservations by the user
        cursor.execute(
        ...
        # Ask the user which reservation to cancel
...
def main_menu():
    ...
    print("14. Cancel Room Reservation")  # New option
...
def interactive_system():
    ...
    while True:
        ...
        elif choice == "7":  # Search for a Book
            search_books()

        elif choice == "8":  # Form a Study Group
            if user:
                form_study_group()
            else:
                print("You need to log in to form a study group.")

        elif choice == "9":  # Show All Study Groups
            show_all_study_groups()

        elif choice == "10":  # Join a Study Group
            if user:
                join_study_group(user.user_id)
            else:
                print("You need to log in to join a study group.")

        elif choice == "11":  # Leave a Study Group
            if user:
                leave_study_group(user.user_id)
            else:
                print("You need to log in to leave a study group.")

        elif choice == "12":  # Search for Study Groups by Genre
            search_study_groups()

        elif choice == "13":  # Reserve a Room
            if user:
                reserve_room(user.name)
            else:
                print("You need to log in to reserve a room.")

        elif choice == "14":  # Cancel Room Reservation
            if user:
        ...
        elif choice == "15":  # Show Room Reservations
            show_reservations()
